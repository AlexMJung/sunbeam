...
class TestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.qbo_accounting_client = QBO(self.company_id).client(rate_limit=499)
        self.qbo_payments_client = QBO(self.company_id).client(rate_limit=39)
        self.customer = models.Customer.customers_from_qbo(self.company_id, self.qbo_accounting_client)[0]
        self.item = next((i for i in models.Item.items_from_qbo(self.company_id, self.qbo_accounting_client) if i.price > 0), None)

        qbo_bank_accounts = self.qbo_payments_client.get("{0}/quickbooks/v4/customers/{1}/bank-accounts".format(app.config["QBO_PAYMENTS_API_BASE_URL"], self.customer.id), headers={'Accept': 'application/json'}).json()
        qbo_bank_account = next((b for b in qbo_bank_accounts if b['accountNumber'] == "xxxxxxxxxxxxx1111"), None)
        if qbo_bank_account:
            self.qbo_payments_client.delete(
        ...
        self.bank_account = models.BankAccount(customer=self.customer, name="Name", routing_number="121042882", account_number="11111111111111111", account_type="PERSONAL_CHECKING", phone="6128675309", qbo_client=self.qbo_payments_client)
        self.bank_account.save()

        qbo_cards = self.qbo_payments_client.get("{0}/quickbooks/v4/customers/{1}/cards".format(app.config["QBO_PAYMENTS_API_BASE_URL"], self.customer.id), headers={'Accept': 'application/json'}).json()
        ...
        self.credit_card = models.CreditCard(customer=self.customer, token=token, qbo_client=self.qbo_payments_client)
    ...
    def test_recurring_payment_bank_account(self):
        with app.test_request_context():
            ...
            deposit_account = models.Account.deposit_account_from_qbo(self.company_id, self.qbo_accounting_client)
            models.Deposit(company_id=self.company_id, account=deposit_account, transaction_id=transaction_id, payment=payment, qbo_client=self.qbo_accounting_client).save()
